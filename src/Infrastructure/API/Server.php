...
use OpenCCK\Infrastructure\API\Middleware\AuthorizationMiddleware;
use Throwable;
...
final class Server implements AppModuleInterface {
    ...
    private function __construct(
        ...
        WebMonitorService::getInstance($this->logger);
    }
    ...
    public function start(): void {
        try {
            ...
            $router->addMiddleware(new AuthorizationMiddleware());
            $router->addRoute('GET', '/', HTTPHandler::getInstance($this->logger)->getHandler());
            $router->addRoute('GET', '/metrics', HTTPHandler::getInstance($this->logger)->getHandler());
